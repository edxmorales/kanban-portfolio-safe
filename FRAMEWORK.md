...
**Versión 1.7.2** · Un framework abierto para equipos que reparten personas entre varios proyectos, deuda técnica, soporte y mejoras, con prioridades que cambian cada semana.
...
KPS es una combinación deliberada de **Kanban** (flujo continuo y límites de trabajo en curso a nivel de equipo y de persona) con la **capa de portafolio de SAFe** (Lean Portfolio Management, Portfolio Kanban y priorización WSJF), más un conjunto de mecánicas propias para resolver lo que ninguno de los dos frameworks resuelve solo: la concentración de conocimiento crítico en pocas personas, la congelación silenciosa de iniciativas cuando la capacidad se desvía a lo urgente, y la falta de autoridad clara sobre qué cuenta como "urgente".

No es Scrum, porque Scrum asume que la prioridad se sostiene durante todo un sprint — y en este contexto cambia de un día para otro. No es SAFe completo, porque SAFe está pensado para 50-125 personas por Tren de Valor, una escala que la mayoría de los equipos que necesitan este framework no tiene ni necesita.

**KPS no se aplica directamente sobre un proyecto o una iniciativa.** Antes de abordar cualquier trabajo, se ejecuta primero el [Diagnóstico 360](#0-diagnóstico-360-el-punto-de-partida-obligatorio) (Sección 0): es el punto de entrada obligatorio del framework, no un anexo opcional. Ningún tablero, límite o rol se instala hasta tener ese diagnóstico completo.
...
**Ninguna de las reglas de este framework se instala a ciegas.** Antes de tocar un tablero, definir un límite de WIP o nombrar un rol, hace falta un diagnóstico de 360 grados del estado real del portafolio — no de lo que se cree que está pasando, sino de lo que los datos muestran. Instalar KPS sin este diagnóstico previo es adivinar la configuración (qué límite de WIP, qué umbral de Expedite, cuántos flujos de valor) en vez de calibrarla con evidencia.

El diagnóstico 360 revisa cinco ángulos, en este orden:

**1. Capacidad real por persona, no por proyecto.** Para cada persona activa, cuántos ítems tiene en curso ahora mismo, sin importar en cuántas iniciativas distintas estén repartidos. Esto expone concentración de carga en pocas personas antes de que se vuelva una crisis.

**2. Cumplimiento real de WIP contra el límite declarado.** Si ya existe algún límite de trabajo en curso configurado en la herramienta actual, compararlo contra la cantidad real de ítems activos. La brecha entre el límite y la realidad es, por sí sola, el primer indicador de qué tan bajo control está el sistema hoy.

**3. Estado de cada iniciativa activa.** Cuáles tienen movimiento reciente y cuáles llevan días o semanas sin ningún ítem en curso — sin que nadie lo haya decidido formalmente. Una iniciativa "viva" en el papel pero congelada en la práctica es exactamente el patrón que KPS existe para prevenir.
...
Cada semana cambian las prioridades, entra trabajo nuevo constantemente, y aun así la sensación es la misma: se saca trabajo del atasco y llega más atasco. Hay iniciativas que avanzan y otras que se quedan congeladas sin que nadie lo haya decidido formalmente. Los pull requests se acumulan antes de cada paso a producción. Las pruebas parecen eternamente atrasadas respecto al desarrollo.

Esto ocurre cuando se mezclan cuatro tipos de trabajo muy distintos —proyectos nuevos, deuda técnica, soporte/incidentes y mejoras— en el mismo equipo, sin reglas explícitas sobre cómo compiten por la atención de las mismas personas, y cuando distintas iniciativas comparten un mismo pool de gente sin ninguna vista conjunta de cuánta capacidad tiene cada quien.

## 2. Fundamentos: por qué Kanban + SAFe

Kanban ordena el flujo dentro de un equipo: visualiza el trabajo, limita cuánto puede estar en curso a la vez, y usa clases de servicio para que no todo compita en igualdad de condiciones. Pero Kanban no dice nada sobre qué hacer cuando la misma persona está repartida entre iniciativas distintas que no se ven entre sí.
...
1. **Una sola vista de capacidad, no un tablero por proyecto.** Equivale al Portfolio Kanban de SAFe: todas las iniciativas en una sola vista, para que se vea cuánto tiene encima cada persona sin importar a cuál iniciativa pertenece cada ítem.
2. **Límite de trabajo en curso por persona, no solo por equipo.** Un equipo puede estar "dentro del límite" mientras dos personas cargan la mitad del trabajo. La regla real es un tope individual (1-2 ítems activos), y cuando alguien lo alcanza, lo siguiente espera, se reasigna, o se decide conscientemente pausar algo suyo.
3. **Clases de servicio con comportamiento, priorizadas con WSJF.** Expedite, Fecha fija, Estándar e Intangible, cada una con reglas de límite y de orden — y la decisión de qué entra primero se apoya en costo de demora dividido entre tamaño del trabajo, no en intuición.
...
- **Guardia rotativa ("bombero de la semana"):** un rol semanal rotativo absorbe lo Expedite, en vez de que siempre recaiga sobre las mismas 1-2 personas con más conocimiento — esto fuerza, con el tiempo, que ese conocimiento se reparta.
...
- **Visibilidad radical:** la carga de cada persona y el estado de cada iniciativa son visibles para toda la organización, no solo para quien lidera — la presión social sostiene las reglas mejor que cualquier auditoría top-down.

## 5. Roles y gobierno

KPS no necesita una estructura de roles pesada, pero sí necesita que cada regla tenga un dueño con nombre — sin eso, las reglas se ignoran bajo presión, que es exactamente el problema que KPS busca resolver. Y necesita, además, que ese dueño nunca sea un punto único de falla: por eso cada rol de esta tabla tiene también un respaldo nombrado explícitamente.

| Rol | Responsabilidad | Autoridad | Respaldo |
|---|---|---|---|
| **Responsable de Flujo de Portafolio** | Mantiene la vista única de capacidad actualizada y decide qué pasa cuando alguien llega a su tope de WIP | Puede pausar, reasignar o rechazar la entrada de un nuevo ítem a una persona saturada | Una segunda persona nombrada puede ejercer la misma decisión si el titular no está disponible — sin esto, este rol tendría el mismo bus factor que el framework busca eliminar en el resto del equipo |
| **Aprobador de Clase de Servicio** | Evalúa y aprueba (o rechaza) que un ítem entre como Expedite | Es la única autoridad que puede etiquetar algo como Expedite | Respaldo nombrado con la misma autoridad — este rol nunca queda sin cobertura, ni siquiera un día, porque lo urgente no espera a que alguien vuelva de vacaciones |
...
**Trabajo de mantenimiento del negocio (debe quedarse interno).** Soporte, incidentes, deuda técnica continua y mejoras incrementales sobre un sistema vivo. Este trabajo requiere continuidad de conocimiento y disponibilidad rápida — es exactamente el tipo de trabajo donde un freelance que termina su contrato y se va empeora el bus factor en vez de mejorarlo, porque el conocimiento del sistema se va con la persona.
...
- La capacidad de un freelance entra en la misma vista única de capacidad que el resto del portafolio (punto 1 de los pilares), marcada explícitamente con su fecha de fin de contrato — así el Responsable de Flujo de Portafolio nunca construye una dependencia permanente sobre alguien que va a irse en una fecha conocida.
- Todo freelance entrega documentación de lo construido como parte de su Definición de Hecho, no como algo opcional de última hora — esto es lo que evita que el conocimiento salga por la puerta junto con la persona (ver también el apalancamiento con IA para generar esta documentación en el punto 10).
...
- **La dependencia vive en la vista única de capacidad (pilar 1),** marcada con el flujo de valor del que depende — no solo aparece en el tablero del flujo que la originó.
...
Principio rector de esta sección: **lo que no se mide no mejora ni se ve** — por eso ningún indicador de KPS se mide una sola vez ni queda a discreción de si alguien se acuerda. Todos entran en el mismo ciclo diario/semanal/mensual/trimestral ya definido en el resto del framework, y todos están orientados a un solo objetivo: hacer visible la eficiencia real del sistema, no solo qué tan ocupada se ve la gente.
...
- **% de interrupciones que, revisadas después, realmente ameritaban ser Expedite** — este número es el que permite calibrar con evidencia si la autoridad de clasificación está siendo demasiado permisiva.
...
- **% de ítems que regresan de QA a desarrollo (rework)** — un indicador alto aquí suele señalar que el shift-left del punto 7 no se está cumpliendo de verdad.
...
- **Trimestral:** recalibración completa junto con la sesión de portafolio del punto 6 de la hoja de ruta.

### 9.9 Indicadores para validar el piloto

Estos son los que convierten "creemos que mejoró" en evidencia real (ver la sección de cómo comprobar el framework en el proceso de piloto):
...
- **Generación de documentación para reducir bus factor.** Esto conecta directo con el punto 6.1: un asistente de IA puede ayudar a transformar el conocimiento tácito de las personas que más saben (o de un freelance antes de que termine su contrato) en documentación real, a partir de código, commits y explicaciones grabadas — reduciendo el costo de transferencia de conocimiento que hoy se pierde.
- **Apoyo a QA:** generación de casos de prueba y de scaffolding de pruebas automatizadas a partir de los criterios de aceptación definidos en el refinamiento, para aliviar la presión sobre el equipo de automatización sin saltarse el shift-left del punto 7.
...
8. **Piloto acotado de 4-6 semanas** en dos flujos de valor, con el criterio de decisión de la sección 9.9 ya acordado desde antes de empezar — no se define "qué cuenta como éxito" después de ver los resultados.
...
- **Costo freelance:** normalmente más bajo por hora facturada, pero con un costo oculto que debe sumarse explícitamente: la transferencia de conocimiento al cierre del contrato (ya exigida como regla en 6.1) y el riesgo de bus factor si esa transferencia no ocurre.
...
SAFe (Scaled Agile Framework) es el marco de trabajo más utilizado para escalar prácticas ágiles en organizaciones grandes, combinando principios de Lean, Agile y pensamiento de sistemas en distintas capas: equipo, programa, solución y portafolio. KPS toma prestada únicamente su capa de portafolio; el resto se describe aquí solo como contexto.
